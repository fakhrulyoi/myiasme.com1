<?php
// product_uploader.php - A file to handle Excel file uploads and automatically fill the product form
...
// Function to extract product data from the uploaded Excel file
function extractProductData($filePath, $productName, $startDate, $endDate) {
    try {
        ...
        foreach ($xlsx->rows() as $rowData) {
            ...
            // If date is within range and row contains relevant product data
            if ($date !== null && $date >= $startDate && $date <= $endDate) {
                // Look for product-related entries in this row
                $salesText = implode(' ', $rowData); // Combine all cells for easier searching
                
                // Check if this row is for our product
                if (stripos($salesText, $productCode) !== false || stripos($salesText, str_replace(' - ', ' ', $productName)) !== false) {
        ...
        // Calculate average sale price per unit if units were sold
        $averageSalePrice = ($unitsSold > 0) ? ($totalSales / $unitsSold) : 0;
...
// Function to get product cost from the sidebar list
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['salesFile'])) {
    ...
    $uploadFile = $uploadDir . basename($_FILES['salesFile']['name']);
    
    if (move_uploaded_file($_FILES['salesFile']['tmp_name'], $uploadFile)) {
        // Extract data from the uploaded file
        $productData = extractProductData($uploadFile, $productName, $startDate, $endDate);
...
<head>
    ...
    <title>Product Data Uploader</title>
    ...
    <div class="dashboard-container">
        <nav class="sidebar">
            ...
            <ul>
                ...
                <li><a href="product_uploader.php"><strong>Product Uploader</strong></a></li>
            </ul>
        </nav>
        
        <div class="main-content">
            <header>
                <h1>Product Data Uploader</h1>
            </header>
                ...
                <section>
                    <div class="upload-container">
                        ...
                        <form method="POST" enctype="multipart/form-data">
                            ...
                            <label for="salesFile">Upload Sales Data File (Excel)</label>
                            <input type="file" id="salesFile" name="salesFile" required accept=".xlsx,.xls">
                            
                            <button type="submit" style="margin-top: 15px;">Process Data</button>
                        </form>
                    </div>
                    
                    <?php if (isset($productData) && !isset($productData['error'])): ?>
